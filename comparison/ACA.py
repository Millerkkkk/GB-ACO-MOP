...
class AntColonyOptimizer:
    def __init__(self, instance, energy_model, vehicle_capacity=650,
    ...
    def split_by_capacity(self, route_global):
        routes = []
        current_route = []
        current_load = 0

        for customer in route_global:
            demand = self.instance.nodes[customer].demand
    ...
    def _evaluate(self, flattened_route_local):
        ...
        # 用 capacity 分割
        splitted_routes = self.split_by_capacity(route_global)
        ...
        for route in splitted_routes:
            ...
            travel_cost_list.append(cost_detail.travel_cost)
            dispatch_cost_list.append(cost_detail.dispatch_cost)
            service_cost_list.append(cost_detail.service_cost)
            charging_cost_list.append(cost_detail.charging_cost)
            total_cost += route_cost

        return (total_cost, routes_dist, decoded_routes, total_num_charge, 
                travel_cost_list, dispatch_cost_list, service_cost_list, charging_cost_list)

    def update_pheromones(self, ant_population):
        """
        ...
        # 蒸发 + 更新
        self.pheromone_matrix = (1 - self.rho) * self.pheromone_matrix + delta_tau
    ...
    def run(self):
        ...
        for gen in range(self.num_iter):
            ...
            self.update_pheromones(ant_population)
...
def run(instance, energy_model, aca_params, seed=None):
    ...
    aco = AntColonyOptimizer(instance, energy_model, **aca_params)
    best = aco.run()

    (total_cost, routes_dist, decoded_routes, total_num_charge, 
    travel_cost_list, dispatch_cost_list, service_cost_list, charging_cost_list) = best

    routes_cost = [total_cost]  # ✅ 保证不是空
...
if __name__ == '__main__':
    ...
    for run_id in range(N_RUNS):
        ...
        solver = run(instance, energy_model, aca_params)
