...
import os
import time
from utils import EVRPInstance, plot_routes, plot_clusters, to_int_list, NodeIndexMap
        ...
        for gen in range(self.num_iter):
            ...
            for _ in range(self.num_ants):
                ...
                if ant.cost < self.best_cost:
                    ...
                    best_raw_route = copy.deepcopy(ant.ant_route)   # 这里保存原始路径

            self.update_pheromones(ant_population, self.best_ant)

            # 记录最优适应度和代价
            fitness_list.append(self.best_cost)

        return best, self.mapper.to_global(best_raw_route)
...
def run(instance, clusters, energy_model, aca_params, max_iter=30, patience=5, removal_ratio=0.3, seed=None):
    rng = np.random.default_rng(seed)
    localSearch = LocalSearch(instance.nodes, instance.distance_matrix, instance.depot_ids, removal_ratio, loc_penalty=0.1)
    
    best_solution = None          # 全局最优
    best_cost = float("inf")
    no_improve = 0
    ...
    for iter in range(1, max_iter + 1):
        ...
        # === B) 局部扰动（跨簇 LS）===
        flat_route, LS_routes, removed_customers = \
            localSearch.fit_only_cross_clusters(raw_routes, vehicle_capacity=650)
        
        clusters = {i: route for i, route in enumerate(LS_routes)}  # cluster id 重置
        ...
        if no_improve >= patience:
            # print(f"Early stopping at iter {iter} (no improvement for {patience} iters).")
            break
...
if __name__ == '__main__':
    ...
    for run_id in range(N_RUNS):
        ...
        instance_clusters = getattr(clusters_result, instance_name)
